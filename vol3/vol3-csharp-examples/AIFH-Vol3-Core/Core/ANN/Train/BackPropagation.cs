...
namespace AIFH_Vol3_Core.Core.ANN.Train
{
    ...
    public class BackPropagation: IGradientCalcOwner, ILearningMethod
    {
        ...
        public void Iteration()
        {
            ...
            foreach (ILayer layer in _network.Layers )
            {
            ...
            int iterationSize = BatchSize == 0 ? _training.Count
            ...
            for (int i = 0; i < iterationSize; i++)
            {
                ...
                if (IsOnlineTraining)
                {
                    if (Stochastic != null)
                    {
                        int stochasticIndex = Stochastic.NextInt(0, _training.Count);
            ...
            for (int i = 0; i < _network.Weights.Length; i++)
            {
                double delta;
        ...
        public bool IsOnlineTraining
        {
            get
            {
                return BatchSize != 0 && (BatchSize < _training.Count);
            }
        }
