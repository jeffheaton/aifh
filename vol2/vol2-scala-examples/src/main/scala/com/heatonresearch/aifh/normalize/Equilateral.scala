...
class Equilateral(count: Int, low: Double, high: Double) extends Serializable {
   ...
   */
  private final val matrix: Array[Array[Double]] = equilat(count, low, high)
   ...
   */
  final def decode(activations: Array[Double]): Int = {
    var minValue: Double = Double.PositiveInfinity
    var minSet: Int = -1
    for(i <- 0 until matrix.length) {
      val dist: Double = getDistance(activations, i)
      if (dist < minValue) {
        minValue = dist
        minSet = i
      }
   ...
   */
  final def encode(set: Int): Array[Double] = {
   ...
   *         set.
   */
  private def equilat(n: Int, low: Double, high: Double): Array[Array[Double]] = {
    var r: Double = .0
    var f: Double = .0
    val result = Array.ofDim[Double](n, n - 1)
    result(0)(0) = -1
    result(1)(0) = 1.0
    for(k <- 2 until n) {
      r = k
   ...
   * @return The distance.
   */
  final def getDistance(data: Array[Double], set: Int): Double = {
    var result: Double = 0
