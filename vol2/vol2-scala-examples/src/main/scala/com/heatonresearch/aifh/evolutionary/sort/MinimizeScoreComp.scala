 ...
 */
@SerialVersionUID(1L)
class MinimizeScoreComp extends AbstractGenomeComparator with Serializable {

  override def compare(p1: Genome, p2: Genome): Int = p1.score.compare(p2.score)

  override def isBetterThan(prg: Genome, betterThan: Genome): Boolean =
    isBetterThan(prg.score, betterThan.score)
