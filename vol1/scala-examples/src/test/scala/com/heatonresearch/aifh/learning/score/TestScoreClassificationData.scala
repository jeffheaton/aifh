 ...
 */
object TestScoreClassificationData {
...
class TestScoreClassificationData extends Suite with ShouldMatchers {
  import TestScoreClassificationData._
  def testClassification() {
    val ACTUAL = Vector(0.0, 1.0, 0.0, 0.0)
    val training = BasicData.convertArrays(TEST_INPUT, TEST_IDEAL)
    val score: ScoreClassificationData = new ScoreClassificationData(training)
    val simple: SimpleAlgo = new SimpleAlgo(ACTUAL)
    val s: Double = score.calculateScore(simple)
