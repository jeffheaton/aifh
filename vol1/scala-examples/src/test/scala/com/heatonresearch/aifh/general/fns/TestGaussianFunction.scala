 ...
 */
class TestGaussianFunction extends Suite with ShouldMatchers {
  def testEvaluate() {
    val params = ArrayBuffer(5.0, 0.0, 0.0, 0.0)
    val funct = new GaussianFunction(3, params, 0)
    val x = Vector(-1.0, .0, 1.0)
    val y = funct.evaluate(x)
    y should be (0.9607894391523232 plusOrMinus AIFH.DEFAULT_PRECISION)
  }
  ...
  def testToString() {
    ...
    funct.evaluate(x)
    assert("[GaussianFunction:width=5.00,center=0.00,0.00,0.00]" === funct.toString)
  }

  def testOther() {
    val params = ArrayBuffer(5.0, 0.0, 0.0, 0.0)
    val funct = new GaussianFunction(3, params, 0)
    assert(funct.getDimensions === 3)
