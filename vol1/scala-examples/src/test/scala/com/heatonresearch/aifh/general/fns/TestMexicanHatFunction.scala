...
class TestMexicanHatFunction extends Suite with ShouldMatchers {
  ...
  def testEvaluate() {
    ...
    val funct = new MexicanHatFunction(3, params, 0)
    val x = Vector(-1.0, 0.0, 1.0)
    val y = funct.evaluate(x)
    y should be (-0.36787944117144233 plusOrMinus AIFH.DEFAULT_PRECISION)
  }
  ...
  def testToString() {
    ...
    val x = Vector(-1.0, 0.0, 1.0)
    funct.evaluate(x)
    assert("[MexicanHatFunction:width=5.00,center=0.00,0.00,0.00]" === funct.toString)
  }
